WITH customer_ages AS (
    SELECT
        ...
        EXTRACT(YEAR FROM now()) - EXTRACT(YEAR FROM c.birthday)
        - CASE WHEN
            (EXTRACT(MONTH FROM now()) < EXTRACT(MONTH FROM c.birthday))
            OR (EXTRACT(MONTH FROM now()) = EXTRACT(MONTH FROM c.birthday) AND EXTRACT(DAY FROM now()) < EXTRACT(DAY FROM c.birthday))
          THEN 1 ELSE 0 END AS customer_age_years
    FROM
        customers c
)
SELECT
