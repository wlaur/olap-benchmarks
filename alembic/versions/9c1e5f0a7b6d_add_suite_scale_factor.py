...
def _validate_legacy_suite_names() -> None:
    quoted_suites = ", ".join(f"'{suite}'" for suite in KNOWN_LEGACY_SUITES)
    unknown_suites = [
        str(row[0])
        ...
        .exec_driver_sql(
            ...
            FROM run
            WHERE suite NOT IN ({quoted_suites})
            ORDER BY suite
            """
        )
        .all()
    ...
    if unknown_suites:
        raise RuntimeError("Cannot infer suite scale factors for unknown legacy suites: " + ", ".join(unknown_suites))


def upgrade() -> None:
    _validate_legacy_suite_names()
    op.execute(
        """
        ...
        SELECT
            ...
            CASE
                ...
                WHEN suite IN ('tpch', 'tpc_h') THEN 10
                ELSE 1
            END,
            db,
            db_version,
...
def downgrade() -> None:
    ...
    op.execute(
        ...
        SELECT
            id,
            CASE
                WHEN suite = 'tpc_h' THEN 'tpch_sf' || suite_scale_factor::VARCHAR
                WHEN suite = 'tpc_ds' THEN 'tpcds_sf' || suite_scale_factor::VARCHAR
                ELSE suite
            END,
            db,
            db_version,
        ...
        FROM run;
        DROP TABLE run;
        ALTER TABLE run_old RENAME TO run;
        """
    )
    op.create_index("idx_run_status_started_at", "run", ["status", "started_at"], unique=False)
