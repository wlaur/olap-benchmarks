...
from ...suites.tpc_h.config import TPCH_TABLES, TpcH
from .. import Database
from .clickbench import execute_clickbench_query
from .tpc_h import execute_tpc_h_query
...
class PolarsTpcH(TpcH["Polars"]):
    def load_tpch_query(self, query_name: str) -> str:
        return f"Polars LazyFrame expression {query_name}"
    ...
    def populate(self, restart: bool = True) -> None:
        ...
        for table_name in TPCH_TABLES:
            with self.db.phase_context("register", table_name=table_name):
                self.db.register_table(table_name, self.input_data_directory / f"{table_name}.parquet")
...
class Polars(Database):
    ...
    def fetch(
        ...
        match self.current_suite:
            ...
            case "tpc_h":
                lf = execute_tpc_h_query(
                    {table_name: self._scan_table(table_name) for table_name in TPCH_TABLES},
    ...
    def suite_registry(self) -> Mapping[SuiteName, type[BenchmarkSuite[Any]]]:
        return {"clickbench": PolarsClickbench, "jsonbench": PolarsJSONBench, "tpc_h": PolarsTpcH}
