...
from .insert import DEFAULT_BATCH_SIZE
from .utils import create_table, get_table

_LOGGER = logging.getLogger(__name__)
_TARGET_ARROW_BATCH_BYTES = 64 * 1024 * 1024
_ESTIMATED_COLUMN_VALUE_BYTES = 8
...
def _get_rows_per_batch(
    schema: Mapping[str, pl.DataType | type[pl.DataType]],
    batch_size: int,
) -> int:
    estimated_row_bytes = max(1, len(schema)) * _ESTIMATED_COLUMN_VALUE_BYTES
    return min(batch_size, max(1, _TARGET_ARROW_BATCH_BYTES // estimated_row_bytes))
...
def _iter_arrow_batches(
    ...
    for frame in df.collect_batches(chunk_size=rows_per_batch):
        row_counter[0] += frame.height
...
def insert_adbc(
    ...
    if isinstance(df, pl.DataFrame):
        ...
        rows_per_batch = _get_rows_per_batch(schema, batch_size)
        data = pa.RecordBatchReader.from_batches(
            schema.to_arrow(),
            _iter_arrow_batches(df, rows_per_batch, row_counter),
        )
    ...
    _LOGGER.info(
        ...
        f"into table {table} with ADBC in {perf_counter() - t0:_.2f} seconds"
    )
...
def upsert_adbc(
    ...
    t0 = perf_counter()
    destination = get_table(table, df.schema)
    source_name = f"_temporary_{uuid.uuid4().hex[:8]}"
    source = create_table(source_name, df.schema, connection, primary_key=primary_key, temporary=True)
    ...
    try:
        ...
        if inserted_rows != df.height:
            raise RuntimeError(f"ADBC reported {inserted_rows:_} inserted rows for a {df.height:_}-row upsert dataset")
    ...
    _LOGGER.info(
        f"Upserted dataset with shape ({df.height:_}, {df.width:_}) "
        f"into table {table} with ADBC in {perf_counter() - t0:_.2f} seconds"
    )
