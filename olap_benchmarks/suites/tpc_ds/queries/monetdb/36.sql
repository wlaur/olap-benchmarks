WITH results AS
  (SELECT sum(ss_net_profit) AS ss_net_profit,
          ...
          cast(sum(ss_net_profit) as double)/sum(ss_ext_sales_price) AS gross_margin ,
          i_category ,
          i_class ,
          0 AS g_category,
          0 AS g_class
  ...
  (SELECT gross_margin,
          ...
          0 AS lochierarchy
   FROM results
   UNION SELECT cast(sum(ss_net_profit) as double)/sum(ss_ext_sales_price) AS gross_margin,
                i_category,
                NULL AS i_class,
                0 AS t_category,
                1 AS t_class,
