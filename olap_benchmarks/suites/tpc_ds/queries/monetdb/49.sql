     ...
     (SELECT item,
      ...
      FROM
        (SELECT ws.ws_item_sk AS item,
                (cast(sum(coalesce(wr.wr_return_quantity,0)) as double)/ cast(sum(coalesce(ws.ws_quantity,0)) as double)) AS return_ratio,
                (cast(sum(coalesce(wr.wr_return_amt,0)) as double)/ cast(sum(coalesce(ws.ws_net_paid,0)) as double)) AS currency_ratio
         FROM web_sales ws
     ...
     (SELECT item,
      ...
      FROM
        (SELECT cs.cs_item_sk AS item,
                (cast(sum(coalesce(cr.cr_return_quantity,0)) as double)/ cast(sum(coalesce(cs.cs_quantity,0)) as double)) AS return_ratio,
                (cast(sum(coalesce(cr.cr_return_amount,0)) as double)/ cast(sum(coalesce(cs.cs_net_paid,0)) as double)) AS currency_ratio
