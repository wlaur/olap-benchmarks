-- Adapted from the official StarRocks TPC-DS benchmark DDL
-- (https://docs.starrocks.io/docs/3.4/benchmarking/TPC_DS_Benchmark/):
-- replication_num 1 (single node), bucket counts scaled down from their
-- 1 TB cluster (capped at 16), and BIGINT for the key columns generated
-- as Int64 in the source Parquet. Their column reordering (DUPLICATE KEY
-- sort columns must lead the table) is kept.
...
DISTRIBUTED BY HASH (cr_item_sk) BUCKETS 16
PROPERTIES ("replication_num" = "1");
...
DISTRIBUTED BY HASH (cs_item_sk) BUCKETS 16
PROPERTIES ("replication_num" = "1");
...
DISTRIBUTED BY HASH (inv_item_sk) BUCKETS 16
PROPERTIES ("replication_num" = "1");
...
DISTRIBUTED BY HASH (sr_item_sk) BUCKETS 10
PROPERTIES ("replication_num" = "1");
...
DISTRIBUTED BY HASH (ss_item_sk) BUCKETS 16
PROPERTIES ("replication_num" = "1");
...
DISTRIBUTED BY HASH (wr_item_sk) BUCKETS 10
PROPERTIES ("replication_num" = "1");
...
DISTRIBUTED BY HASH (ws_item_sk) BUCKETS 16
PROPERTIES ("replication_num" = "1");
